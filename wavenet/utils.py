...
def convert_mp3_folder(input_folder: str, sample_rate: float) -> str:
    ...
    logger.info("Converting mp3 files located in [%s] to WAV with sample rate [%d]",
                input_folder, sample_rate)
...
def mp3_to_wav(input_folder: str,
               output_folder: str,
               filename: str,
    ...
    if not os.path.exists(wav_full_path):
        ...
        mp3.export(wav_full_path,
                   format="wav",
                   parameters=["-ar", f"{sample_rate}"])
